...
while ($argcheck < scalar(@ARGV)) {
	...
	$force=1;
    } elsif (lc($ARGV[$argcheck]) eq '--gcc-compile') {
	$gcc_compile=1;
    } else {
...
if (-e $hsiTar) {
	...
	# This isn't recommended, but gets around situations where cmake
	# isn't up-to-date and the user doesn't have the authority to
	# update
	system("mkdir build");
	my $gcc_base = 'gcc -O3 -o build/';

	my $gcc_cmd = $gcc_base.'sindex src/sindex.c src/hsi_index.c';
	if (system($gcc_cmd)) {
	    die "\n  ERROR:  (unrecommended) gcc compile of sindex failed ($gcc_cmd)\n\n";
	}

	$gcc_cmd = $gcc_base.'sfetch src/sfetch.c src/hsi_index.c';
	if (system($gcc_cmd)) {
	    die "\n  ERROR:  (unrecommended) gcc compile of sfetch failed ($gcc_cmd)\n\n";
	}

	$gcc_cmd = $gcc_base.'sstat src/sstat.c src/hsi_index.c';
	if (system($gcc_cmd)) {
	    die "\n  ERROR:  (unrecommended) gcc compile of sstat failed ($gcc_cmd)\n\n";
	}
