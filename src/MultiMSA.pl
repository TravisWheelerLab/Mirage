...
while (!eof($infile)) {
	...
	my $famhitfilename = $outputfolder.'/'.$group_id.'.hits.tmp'; # Call this tmp because we might pull some out (if they mapped to the wrong chr)
	open(my $famhitfile,'>>',$famhitfilename);
	print $famhitfile "$ID_line\n";
	print $famhitfile "$method_line";
	print $famhitfile "$chr_name_line";
	print $famhitfile "$pos_line\n"; # Extra newline for presentation's sake
	close($famhitfile);
	$FamHitFiles{$group_id} = $famhitfilename;
